...
2. ~~브라우저 검증~~ — **완료.** Playwright(자체 Chromium)로 375px 실측·모바일 드로어·
   `DesktopOnly` 차단·드래그앤드롭 지속성까지 22/22 통과. 하네스는 [`e2e/`](../e2e/README.md).
   `BASE_URL`만 바꾸면 어느 인스턴스에나 붙는다.
...
**푸는 방법.** `/admin/people`의 `계정 미연결 · 만들기` 배지를 누르면
`POST /admin/people/:id/create-account`가 계정을 만들고 프로필에 바로 연결한다.
`system` 기능 역할이 필요하다(계정 생성이므로 `POST /admin/users`와 같은 게이트).

**비밀번호는 관리자가 정하지 않는다.** 계정은 `is_active=false` + 추측 불가능한
난수 해시로 만들어지고 활성화 링크가 함께 발급된다. 본인이 그 링크로 직접 정한다 —
`convert-to-student`와 같은 방식이다. 실제 사람에게 관리자가 비밀번호를 만들어
주는 흐름은 두지 않는다.

활성화 링크는 **생성 직후 한 번만** 보인다(서버는 해시만 저장). 놓치면
`POST /admin/users/:id/activation-token`으로 재발급한다.

수동 2단계(`/admin/users` 생성 → `/admin/people` 연결)도 그대로 가능하다.
이미 있는 계정을 연결할 때는 그쪽을 쓴다 — 신규 생성 API는 이메일이 이미 쓰이면
409로 거부하고 그쪽을 안내한다.
