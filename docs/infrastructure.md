...
아무것도 보장하지 않는다. 2026-08-20 까지 이 저장소의 백업은 **한 번도 복원돼 본 적이
없었다** — "백업이 있다" 가 검증되지 않은 약속이었다.

`ops/verify-restore.sh` 가 세 갈래를 다 본다. 하나라도 안 되면 그 갈래는 없는 것과 같다.

| | 무엇을 | 왜 |
|---|---|---|
| ① 평문 덤프 | `zcat` → `psql` → **43개 테이블 행수를 라이브와 대조** | 빠른 복원용. "복원됐다" 와 "제대로 복원됐다" 는 다르다 |
| ② 암호화본 | `gpg` → `zcat` → `psql` | 밖으로 내보낼 때 쓰는 것. 못 푸는 암호문은 백업이 아니다 |
| ③ 업로드 tar | 풀어서 **바이트 단위 비교** | DB 에는 경로만 있다. 덤프만 복원하면 주소는 살아나고 그림이 전부 깨진다 |

행수는 `pg_stat_user_tables.n_live_tup` 이 아니라 **실제 `count(*)`** 로 센다 — 전자는
근사값이라 대조에 못 쓴다.

라이브를 건드리지 않는다. 사본 DB 를 만들어 거기에 복원하고 끝나면 지우므로 아무 때나
돌려도 된다. `seeds-restore-check.timer` 가 **일요일 06:00** 에 돌린다(백업 04:30·검증
05:15 이 끝난 뒤). 매일이 아닌 이유는 사본 DB 를 두 개 만들었다 지우므로 백업보다 무겁고,
"복원이 되는가" 는 매일 바뀌는 성질이 아니기 때문이다.

첫 실측(2026-08-20): 43개 테이블 행수 완전 일치 · 암호화본 복호화 성공 · 업로드 파일
바이트까지 동일.
