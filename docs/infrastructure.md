...
`ops/verify-restore.sh` 가 **두 가지 질문을 나눠서** 묻는다.
...
| ② 암호화본 | `gpg` → `zcat` → `psql` → **테이블 수 대조** | 밖으로 내보낼 때 쓰는 것. 못 푸는 암호문은 백업이 아니다 |
| ③ 업로드 tar | 풀어서 **바이트 단위 비교** | DB 에는 경로만 있다. 덤프만 복원하면 주소는 살아나고 그림이 전부 깨진다 |

**담기는가** — 백업이 DB 내용을 그대로 담는가.

| | 무엇을 | 왜 |
|---|---|---|
| ④ 갓 뜬 백업 | 지금 백업 → 복원 → **43개 테이블 행수 대조** | "복원됐다" 와 "제대로 복원됐다" 는 다르다 |

**④ 는 반드시 갓 뜬 백업으로 물어야 한다.** 처음엔 보관된 백업을 복원해 *지금* 라이브와
행수를 맞댔는데, 백업 이후 검증 주행이 만든 행 때문에 **항상 어긋났다**(2026-08-26 발견).
그건 백업 결함이 아니라 질문이 틀린 것이었고, 그대로 뒀으면 일요일마다 거짓 경보를
울렸을 것이다. **거짓 경보를 울리는 검사는 없느니만 못하다** — 사람들이 그 신호를
무시하게 된다.
