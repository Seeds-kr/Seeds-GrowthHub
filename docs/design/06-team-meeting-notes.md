...
학생이 스터디를 **열자고 할 통로가 없었다.** `admin-studies.ts`에만 쓰기 경로가 있고 `requireStudent`는 0건이라, 학생 화면은 남이 연 스터디를 구경하는 것뿐이었다.

### ADR-012 — 요청은 별도 테이블이 아니라 `studies.status`

**결정.** `proposed` · `rejected`를 `STUDY_STATUSES` 앞에 붙인다. `study_requests`를 만들지 않는다.

**근거.** **승인된 요청이 곧 그 스터디다** — 학생이 쓴 제목·주제·주차 계획이 그대로 굴러간다. 별도 테이블이면 승인 시점에 그것을 옮겨 적고 두 행을 맞춰 나가야 하는데, 그게 `applications`가 이미 후회하고 있는 모양이다([gap-register §4](../gap-register.md) "이중 상태").

**대가.** `studies` 조회가 상태를 항상 의식해야 한다. 그래서 `STUDY_PUBLIC_STATUSES`를 따로 둔다.

### 가시성 — 여기가 조심할 곳

`/student/studies`는 원래 **같은 기수 스터디를 전부** 돌려줬다. 여기에 상태만 추가하면 심사 중인 제안과 **반려 사유가 기수 전체에 공개된다.** 반려 사유는 제안자에게 쓴 글이지 동료에게 쓴 글이 아니다.

그래서 조회를 갈랐다:

| 분기 | 무엇 |
|---|---|
| 멤버십 / 본인이 leader | 상태 무관 — 내 제안·반려가 여기로 온다 |
| 같은 기수 | `STUDY_PUBLIC_STATUSES`만 (`planned`·`active`·`completed`·`archived`) |

목록과 상세 **양쪽**에 같은 규칙을 건다. 상세만 빠뜨리면 id를 찍어 남의 반려 사유를 읽을 수 있다.

### 심사

| | |
|---|---|
| 승인·반려 | `requireOpsRole("growth")` — [00 §2.3](00-target-state.md)이 성장경험 담당에게 스터디를 배정했다 |
| 열람 | 모든 운영진. 막는 것은 결정뿐이다 |
| 반려 사유 | **필수(422).** "안 됩니다"로 끝나면 학생은 무엇을 고쳐 다시 낼지 모르고, 대개 다시 내지 않는다. 반려는 종료가 아니라 되돌려 보내는 것이다 |
| 동시 제안 | 1건(409). 심사 큐는 사람의 주의력이다 |
| 재심사 방지 | `proposed`가 아닌 것을 승인하면 409 — 진행 중인 스터디가 `planned`로 되돌아가는 것을 막는다 |

승인되면 제안자가 leader이자 첫 멤버로 들어간다. 안 그러면 승인된 스터디에 아무도 없는 채로 시작한다.
