...
- [x] 담당 프로젝트가 없는 멘토가 `/mentor/teams`에서 빈 목록 + 안내 문구를 본다 (에러 아님).
      → 2026-08-24 실측. 임시 멘토 계정으로 `{"items":[],"total":0}` + **HTTP 200**.
...
- [x] `status='ended'` 처리 즉시 해당 프로젝트 접근이 404가 된다.
      → 2026-08-24 실측. 해제 전 200 → 해제 후 **404**, 목록도 빈다. 행은 지워지지
      않고 `status='ended'` + `ended_at` 으로 남는다(ADR-003).
      **`DELETE /admin/projects/:id/mentors/:assignmentId` 의 마지막 인자는 배정 행의
      id 다** — 멘토의 user id 를 넣으면 404 가 나는데, 그게 "해제됐다" 로 보이기 쉽다.
...
- [x] 한 프로젝트에 멘토 2명 배정이 가능하고, 양쪽 모두 접근된다.
      → 2026-08-24 실측. project 2 에 멘토 둘(14·17)이 `active` 로 붙고 양쪽 다 200.
- [x] 학생 라우트 회귀 없음 — [visibility-policy §6](../visibility-policy.md) 체크리스트 통과.
      → §6 은 일회성 시험이 아니라 **학생 라우트를 건드릴 때마다 적용하는 리뷰
      체크리스트**다. 여기서 확인할 수 있는 것은 "이 변경이 학생 축을 깨뜨리지
      않았는가" 이고, 그건 스토리 S1~S9 가 매 검증에서 통과하는 것으로 본다 —
      특히 **S6**(남의 자료 접근 → 자기 화면으로 되돌림, 노출 0)과
      **S7**(남의 팀 회의록 차단), **S9**(사진 업로드 권한 밖 403).
- [x] `admin`이면서 `mentor`인 사용자가 두 workspace를 오갈 때 각각의 규칙이 독립 적용된다.
      → 2026-08-24 실측. `role=admin` + `extraRoles=[mentor]` + `program_lead` 계정으로
      어드민 라우트는 전부 200, 그런데 `/mentor/projects/2` 는 **404** 였다.
      **전권 어드민이어도 멘토 화면에서는 배정이 없으면 못 본다** — 멘토 축의 접근은
      역할이 아니라 배정으로 갈린다(ADR-003).
