  ...
  return (
    <AdminLayout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <Card className="rounded-none"><CardHeader><CardTitle className="text-sm">학생 수</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{data.studentCount}</CardContent></Card>
        <Card className="rounded-none"><CardHeader><CardTitle className="text-sm">프로젝트 수</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{data.projectCount}</CardContent></Card>
        <Card className="rounded-none"><CardHeader><CardTitle className="text-sm">아티팩트 수</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{data.artifactCount}</CardContent></Card>
        <Card className="rounded-none"><CardHeader><CardTitle className="text-sm">출석 기록</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{data.attendanceOverview.total}</CardContent></Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <Card className="rounded-none md:col-span-2">
          ...
          <CardContent>
            ...
            : <div className="flex flex-wrap gap-1">{data.skillTagDistribution.map((t) => <Badge key={t.tagId} variant="outline" className="rounded-none">{t.name} · {t.count}</Badge>)}</div>}
          </CardContent>
        </Card>
        <Card className="rounded-none md:col-span-2">
          ...
          <CardContent className="text-sm">
            ...
            : <div className="flex flex-wrap gap-2">{data.studentsMissingActivity.map((s) => (
                <Link key={s.id} href={`/admin/students/${s.id}`}><Badge variant="outline" className="rounded-none cursor-pointer">{s.name}</Badge></Link>
              ))}</div>}
          </CardContent>
        </Card>
      </div>
