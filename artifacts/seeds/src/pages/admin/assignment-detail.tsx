...
function FeedbackBlock({ sub }: { sub: Submission }) {
  ...
  return (
    <div className="space-y-2">
      <Textarea className="" placeholder="피드백…" value={feedback} onChange={(e) => setFeedback(e.target.value)} />
      <Button size="sm" className="" disabled={save.isPending} onClick={() => save.mutate()}>저장 & 검토 완료</Button>
    </div>
  );
}
