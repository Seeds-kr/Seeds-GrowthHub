      ...
      <div className="space-y-4">
          ...
          <CardContent className="text-sm space-y-1">
            ...
            : data.submissions.map((s) => (
              <div key={s.id} className="flex justify-between border-b border-border py-1">
                <span>{s.title}</span>
                <span><Badge variant="outline" className="rounded-none">{s.status}</Badge></span>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="rounded-none">
          ...
          <CardContent className="text-sm space-y-1">
            ...
            : data.projects.map((p) => (
              <div key={p.id}>{p.title} · <Badge variant="outline" className="rounded-none">{PROJECT_STATUS_LABEL[p.status]}</Badge> · {p.role ?? "-"}</div>
            ))}
          </CardContent>
        </Card>
