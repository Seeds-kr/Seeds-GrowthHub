...
# 있다" 가 검증되지 않은 약속이다.
#
# **두 가지 질문을 나눠서 묻는다.** 처음엔 섞어서 물었다가 매주 거짓 경보를
# 울릴 뻔했다(2026-08-26) — 보관된 백업을 복원해 **지금** 라이브와 행수를 맞대니,
# 백업 이후 검증 주행이 만든 행 때문에 항상 어긋났다. 그건 백업 결함이 아니다.
#
#   열리는가   보관된 최신 백업이 오류 없이 복원되는가. 파일이 상했으면 여기서 걸린다.
#              ① 평문 덤프  ② 암호화본  ③ 업로드 tar
#
#   담기는가   지금 뜬 백업이 지금 DB 를 그대로 담는가. **갓 뜬 백업**으로만
#              물을 수 있다 — 시간이 지나면 라이브가 움직이므로.
#              ④ 방금 백업 → 복원 → 컬럼·행수 대조
#
# 거짓 경보를 울리는 검사는 없느니만 못하다. 사람들이 그 신호를 무시하게 된다.
#
...
if [ ! -f "$GPG" ]; then
  ...
  if gpg --batch --quiet --decrypt --passphrase-file "$PASS_FILE" "$GPG" 2>/dev/null \
    ...
    # 테이블 **수**로 본다. 행수와 달리 이건 시간이 지나도 안 변하므로 보관된
    # 백업에도 물을 수 있다. 내용 대조는 ④ 가 갓 뜬 백업으로 한다.
    G_N="$(psql_ "$GPROBE" -tAc "select count(*) from information_schema.tables where table_schema='public';")"
    LIVE_N="$(psql_ "$LIVE_DB" -tAc "select count(*) from information_schema.tables where table_schema='public';")"
    if [ "$G_N" = "$LIVE_N" ]; then ok "복호화·복원됨 ($G_N개 테이블)"
    else bad "테이블 수가 다르다 ($G_N ≠ $LIVE_N) — 스키마가 그 사이 바뀌었는지 확인하라"; fi
  else
    bad "복호화 또는 복원 실패"
  fi
fi
...
echo
echo "④ 갓 뜬 백업이 지금 DB 를 담는가"
FRESH_DIR="$(mktemp -d)"
FRESH="$FRESH_DIR/now.sql.gz"
if docker exec "$CONTAINER" pg_dump -U growthhub -d "$LIVE_DB" --clean --if-exists 2>/dev/null | gzip > "$FRESH" && gzip -t "$FRESH"; then
  FPROBE="fresh_probe_$$"
  drop_ "$FPROBE"; psql_ postgres -tAc "CREATE DATABASE $FPROBE;" >/dev/null 2>&1
  if zcat "$FRESH" | psql_ "$FPROBE" >/dev/null 2>&1; then
    # 행수는 실제 count(*) 로 센다. pg_stat 의 n_live_tup 은 근사값이라 대조에 못 쓴다.
    ...
    B="$(psql_ "$FPROBE"  -tAF: -c "$Q" 2>/dev/null | sort)"
    if [ "$A" = "$B" ]; then
      ok "$(echo "$TABS" | wc -w)개 테이블 행수가 라이브와 일치"
    else
      bad "갓 뜬 백업인데 행수가 다르다 — 덤프가 내용을 잃고 있다"
      diff <(echo "$A") <(echo "$B") | head -10 | sed 's/^/      /'
    fi
  else
    bad "갓 뜬 백업을 복원하지 못했다"
  fi
  drop_ "$FPROBE"
else
  bad "백업을 뜨지 못했다"
